...
function Filter() {
  ...
  return (
    <div className="flex flex-col mt-10">
      <div className="mb-4">
        <h2 className="font-bold text-base text-gray-600">Categorys</h2>
      ...
      <div className="mb-4">
        <h2 className="font-bold text-base text-gray-600">Companys</h2>
        <div className="flex flex-col my-5">
          {companys &&
            companys.map((value) => (
              <p
                key={value}
                className={`${
                  value == activeCompany && styles.active_filter
                } text-gray-500 cursor-pointer mb-2`}
                onClick={() => filterCategory(value, "company")}
              >
                {value}
              </p>
            ))}
      ...
      <div className="mb-4">
        <h2 className="font-bold text-base text-gray-600">Colors</h2>
        <div className="flex justify-between my-5">
          {colors &&
            colors.map((value) => (
              <div
                onClick={() => filterCategory(value, "colors")}
                key={value}
                className={`w-7 h-7 cursor-pointer border-4  shadow-sm ${
                  value == activeColor ? "border-white" : "border-gray-200"
                } rounded-full mx-1`}
